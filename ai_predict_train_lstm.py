#!/usr/bin/env python3
# ai_predict_train_lstm.py
# Copyright Su Nie | BSD-3C License | https://github.com/can87683

import setproctitle
setproctitle.setproctitle("ai_predict_train_lstm")
...
class Config:
    ...
    # Model settings (LSTM)
    DEFAULT_HORIZON = 10
    DEFAULT_SEQUENCE_LENGTH = 30        # lookback window fed to the LSTM
    DEFAULT_EPOCHS = 100
    ...
    # Output naming
    MODEL_PREFIX = "ai_predict_lstm"
    MODEL_EXT = ".pt"            # PyTorch state dict
    METADATA_EXT = ".pkl"

    # UI settings
    APP_TITLE = "🚀 LSTM Model Trainer"
...
class StreamingLSTMTrainer:
    def __init__(self, log_callback=None, timeframe=None):
        ...
        self._log(f"🚀 LSTM trainer{tf_label} initialized. Device attempt order: {preferred.upper()}")
    ...
    def _run_training_loop(self, train_loader, val_loader, num_features, num_groups, hidden_size, num_layers, dropout, embedding_dim, epochs, learning_rate, device, timeframe, status_callback):
        model = LSTMRegressor(num_features, hidden_size, num_layers, dropout, num_groups, embedding_dim).to(device)
    ...
    def save_model(self, model_dir, filename_stem, feature_cols, horizon, timeframe, num_groups, group_id_categories, sequence_length):
        ...
        try:
            ...
            metadata = {
                "model_type": "torch.LSTMRegressor", "timeframe": timeframe, "model_path": str(model_path),
            ...
            self._log(f"✅ LSTM model saved: {model_path}")
            return True
        except Exception as exc:
...
class LSTMTrainerGUI(CTk):
    ...
    def _create_ui(self):
        ...
        CTkLabel(title_frame, text="🎯 LSTM Model Trainer (per timeframe)", font=("Ubuntu", 20, "bold")).pack(pady=4)
    ...
    def start_training(self):
        ...
        def training_task():
            try:
                ...
                self.gui_logger.log("🚀 Starting Per-Timeframe LSTM Training")
                ...
                for timeframe in selected_timeframes:
                    ...
                    self.update_status(f"[{timeframe}] Training LSTM model...")
                    trainer = StreamingLSTMTrainer(self.gui_logger.log, timeframe=timeframe)
