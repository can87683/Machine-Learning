#!/usr/bin/env python3
# ai_predict_train_patchtst_ctk.py
# BSD-C-3 License

import setproctitle
setproctitle.setproctitle("ai_predict_train_patchtst_ctk")
...
class Config:
    ...
    # Model settings (PatchTST)
    DEFAULT_HORIZON = 10
    DEFAULT_SEQUENCE_LENGTH = 30        # lookback window fed to the PatchTST encoder
    DEFAULT_EPOCHS = 100
    DEFAULT_LEARNING_RATE = 0.001
    DEFAULT_PATCH_LEN = 5                # timesteps per patch token
    DEFAULT_PATCH_STRIDE = 5             # 5/5 = non-overlapping patches; use < patch_len to overlap
    DEFAULT_D_MODEL = 64                 # transformer embedding width (must be divisible by DEFAULT_NHEAD)
    DEFAULT_NHEAD = 4
    ...
    # Output naming
    MODEL_PREFIX = "ai_predict_patchtst"
    MODEL_EXT = ".pt"            # PyTorch state dict
    METADATA_EXT = ".pkl"

    # UI settings
    APP_TITLE = "🚀 PatchTST Model Trainer"
...
class PatchTSTRegressor(nn.Module):
    def __init__(self, num_features, seq_len, patch_len, patch_stride,
                 d_model, nhead, num_encoder_layers, dropout,
                 num_groups, embedding_dim):
        super().__init__()

        if seq_len < patch_len:
            raise ValueError(f"sequence_length ({seq_len}) must be >= patch_len ({patch_len})")
        ...
        if self.num_patches < 1:
            raise ValueError(
                f"patch_len/patch_stride combination produces 0 patches for seq_len={seq_len} "
                f"(patch_len={patch_len}, patch_stride={patch_stride})"
            )
        ...
        self.head = nn.Sequential(
            nn.Linear(d_model + embedding_dim, max(d_model // 2, 8)),
            nn.GELU(),
            nn.Dropout(dropout),
            nn.Linear(max(d_model // 2, 8), 1),
        )

    def forward(self, x_seq, group_ids):
        batch, seq_len, num_features = x_seq.shape
        x = x_seq.transpose(1, 2)
        patches = x.unfold(dimension=2, size=self.patch_len, step=self.patch_stride)
        patches = patches.reshape(batch * num_features, self.num_patches, self.patch_len)
        embedded = self.patch_embed(patches)
        embedded = self.embed_dropout(embedded + self.pos_embed)
        encoded = self.encoder(embedded)
        pooled = encoded.mean(dim=1)
        pooled = pooled.reshape(batch, num_features, -1)
        channel_agg = pooled.mean(dim=1)
        emb = self.embedding(group_ids)
        combined = torch.cat([channel_agg, emb], dim=1)
        return self.head(combined).squeeze(-1)


class StreamingPatchTSTTrainer:
    def __init__(self, log_callback=None, timeframe=None):
        ...
        self.patch_len = Config.DEFAULT_PATCH_LEN
        self.patch_stride = Config.DEFAULT_PATCH_STRIDE
        self.d_model = Config.DEFAULT_D_MODEL
        self.nhead = Config.DEFAULT_NHEAD
        self.num_encoder_layers = Config.DEFAULT_NUM_ENCODER_LAYERS
        ...
        self._log(f"🚀 PatchTST trainer{tf_label} initialized. Device attempt order: {preferred.upper()}")
    ...
    def train_single_timeframe(self, data, feature_cols, timeframe, num_groups, epochs=100, learning_rate=0.001,
                               sequence_length=30, patch_len=5, patch_stride=5, d_model=64, nhead=4,
                               num_encoder_layers=2, dropout=0.1, batch_size=128, status_callback=None, **kwargs):
        try:
            self.stop_flag = False
            self.timeframe = timeframe
            ...
            for device in self.gpu_manager.preferred_devices():
                ...
                try:
                    trained_ok = self._run_training_loop(train_loader, val_loader, len(feature_cols), num_groups, sequence_length, patch_len, patch_stride, d_model, nhead, num_encoder_layers, dropout, self.embedding_dim, epochs, learning_rate, device, timeframe, status_callback)
    ...
    def _run_training_loop(self, train_loader, val_loader, num_features, num_groups, sequence_length, patch_len, patch_stride, d_model, nhead, num_encoder_layers, dropout, embedding_dim, epochs, learning_rate, device, timeframe, status_callback):
        model = PatchTSTRegressor(num_features, sequence_length, patch_len, patch_stride, d_model, nhead, num_encoder_layers, dropout, num_groups, embedding_dim).to(device)
    ...
    def save_model(self, model_dir, filename_stem, feature_cols, horizon, timeframe, num_groups, group_id_categories, sequence_length):
        ...
        try:
            ...
            metadata = {
                "model_type": "torch.PatchTSTRegressor", "timeframe": timeframe, "model_path": str(model_path),
                ...
                "architecture": {"num_features": len(feature_cols), "seq_len": sequence_length, "patch_len": self.patch_len, "patch_stride": self.patch_stride, "d_model": self.d_model, "nhead": self.nhead, "num_encoder_layers": self.num_encoder_layers, "dropout": self.dropout, "embedding_dim": self.embedding_dim},
            ...
            self._log(f"✅ PatchTST model saved: {model_path}")
            return True
        except Exception as exc:
...
class PatchTSTTrainerGUI(CTk):
    ...
    def _create_ui(self):
        ...
        CTkLabel(title_frame, text="🎯 PatchTST Model Trainer (per timeframe)", font=("Ubuntu", 16, "bold")).pack(pady=4)
    ...
    def _create_parameters_section(self, parent):
        ...
        self.patch_len = tk.StringVar(value=str(Config.DEFAULT_PATCH_LEN))
        self.patch_stride = tk.StringVar(value=str(Config.DEFAULT_PATCH_STRIDE))
        self.d_model = tk.StringVar(value=str(Config.DEFAULT_D_MODEL))
        self.nhead = tk.StringVar(value=str(Config.DEFAULT_NHEAD))
        self.num_encoder_layers = tk.StringVar(value=str(Config.DEFAULT_NUM_ENCODER_LAYERS))
        ...
        CTkLabel(row2, text="Patch Len:", width=70).grid(row=0, column=0, padx=(0, 3), sticky="w")
        CTkEntry(row2, textvariable=self.patch_len, width=60).grid(row=0, column=1, padx=(0, 10), sticky="w")
        CTkLabel(row2, text="Stride:", width=70).grid(row=0, column=2, padx=(0, 3), sticky="w")
        CTkEntry(row2, textvariable=self.patch_stride, width=60).grid(row=0, column=3, padx=(0, 10), sticky="w")
        CTkLabel(row2, text="D Model:", width=70).grid(row=0, column=4, padx=(0, 3), sticky="w")
        CTkEntry(row2, textvariable=self.d_model, width=60).grid(row=0, column=5, padx=(0, 10), sticky="w")
        CTkLabel(row2, text="Heads:", width=70).grid(row=0, column=6, padx=(0, 3), sticky="w")
        CTkEntry(row2, textvariable=self.nhead, width=60).grid(row=0, column=7, sticky="w")

        row3 = CTkFrame(param_frame, fg_color="transparent")
        row3.pack(fill="x", pady=1)
        for col in range(8): row3.grid_columnconfigure(col, weight=1 if col % 2 else 0)
        CTkLabel(row3, text="Layers:", width=70).grid(row=0, column=0, padx=(0, 3), sticky="w")
        CTkEntry(row3, textvariable=self.num_encoder_layers, width=60).grid(row=0, column=1, padx=(0, 10), sticky="w")
        CTkLabel(row3, text="Batch:", width=70).grid(row=0, column=2, padx=(0, 3), sticky="w")
        CTkEntry(row3, textvariable=self.batch_size, width=60).grid(row=0, column=3, padx=(0, 10), sticky="w")
        CTkLabel(row3, text="Dropout:", width=70).grid(row=0, column=4, padx=(0, 3), sticky="w")
        CTkEntry(row3, textvariable=self.dropout, width=60).grid(row=0, column=5, padx=(0, 10), sticky="w")
    ...
    def start_training(self):
        ...
        def training_task():
            try:
                ...
                self.gui_logger.log("🚀 Starting Per-Timeframe PatchTST Training")
                ...
                patch_len = int(self.patch_len.get())
                patch_stride = int(self.patch_stride.get())
                d_model = int(self.d_model.get())
                nhead = int(self.nhead.get())
                num_encoder_layers = int(self.num_encoder_layers.get())
                ...
                for timeframe in selected_timeframes:
                    ...
                    self.update_status(f"[{timeframe}] Training PatchTST model...")
                    trainer = StreamingPatchTSTTrainer(self.gui_logger.log, timeframe=timeframe)
                    ...
                    success = trainer.train_single_timeframe(
                        ...
                        sequence_length=sequence_length, patch_len=patch_len, patch_stride=patch_stride, d_model=d_model,
                        nhead=nhead, num_encoder_layers=num_encoder_layers, dropout=dropout, batch_size=batch_size,
