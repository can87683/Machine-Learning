...
class Config:
    ...
    # Model settings (1D CNN)
    DEFAULT_HORIZON = 10
    DEFAULT_SEQUENCE_LENGTH = 30
    DEFAULT_EPOCHS = 100
    DEFAULT_LEARNING_RATE = 0.001
    DEFAULT_CHANNELS = [64, 128, 128]
    ...
    TIMEFRAME_HYPERPARAMS = {
        "M5":  {"sequence_length": 30, "channels": [64, 128, 128], "dropout": 0.2},
        "M15": {"sequence_length": 30, "channels": [64, 128, 128], "dropout": 0.2},
        "H1":  {"sequence_length": 24, "channels": [48, 96, 96],   "dropout": 0.25},
        "H4":  {"sequence_length": 20, "channels": [48, 96, 96],   "dropout": 0.25},
        "D1":  {"sequence_length": 14, "channels": [32, 64],       "dropout": 0.3},
        "W1":  {"sequence_length": 8,  "channels": [16, 32],       "dropout": 0.35},
    }
    ...
    # Output naming
    MODEL_PREFIX = "ai_predict_cnn"
    MODEL_EXT = ".pt"
    METADATA_EXT = ".pkl"

    # UI settings
    APP_TITLE = "🚀 CNN Model Trainer"
    APP_GEOMETRY = "640x1050"  # Increased height to fit 3 rows of indicators
    THEME = "dark"
...
class StreamingDataPreparer:
    ...
    def _prepare_single_series(self, df, symbol, horizon, requested_features):
        ...
        # Dynamically Compute Indicators based on GUI Flags
        if self.enable_atr:
...
class QuantilePinballLoss(nn.Module):
    def __init__(self, quantiles):
        super().__init__()
        self.register_buffer("quantiles", torch.tensor(quantiles, dtype=torch.float32).unsqueeze(0))
...
class CNNRegressor(nn.Module):
    def __init__(self, num_features, channels, kernel_size, dropout, num_groups, embedding_dim, num_quantiles):
        super().__init__()
        ...
        self.head = nn.Sequential(
            nn.Linear(in_channels + embedding_dim, max(in_channels // 2, 8)),
            nn.ReLU(), nn.Dropout(dropout),
            nn.Linear(max(in_channels // 2, 8), num_quantiles),
        )
...
class StreamingCNNTrainer:
    def __init__(self, log_callback=None, timeframe=None):
        ...
        self.sequence_length = tf_params.get("sequence_length", Config.DEFAULT_SEQUENCE_LENGTH)
        self.epochs = Config.DEFAULT_EPOCHS
        self.learning_rate = Config.DEFAULT_LEARNING_RATE
        self.channels = list(tf_params.get("channels", Config.DEFAULT_CHANNELS))
        self.kernel_size = Config.DEFAULT_KERNEL_SIZE
        self.dropout = tf_params.get("dropout", Config.DEFAULT_DROPOUT)
        ...
        self._log(f"🚀 CNN trainer{tf_label} initialized. Device attempt order: {preferred.upper()}")
    ...
    def _compute_directional_accuracy(self, model, X_val, g_val, raw_close_val, raw_target_val, device, scaler_state):
        try:
            ...
            median_scaled = np.concatenate(preds_list, axis=0)[:, Config.MEDIAN_QUANTILE_INDEX]
            median_price = median_scaled * scaler_state["target_std"] + scaler_state["target_mean"]
    ...
    def train_single_timeframe(self, data, feature_cols, timeframe, num_groups, epochs=100, learning_rate=0.001,
                               sequence_length=None, channels=None, kernel_size=3, dropout=None, batch_size=128,
                               status_callback=None, auto_tune_architecture=True, **kwargs):
        try:
            self.stop_flag = False
            self.timeframe = timeframe
            ...
            if auto_tune_architecture:
                sequence_length = tf_params.get("sequence_length", sequence_length or self.sequence_length)
                channels = tf_params.get("channels", channels or self.channels)
                dropout = tf_params.get("dropout", dropout if dropout is not None else self.dropout)
            else:
            ...
            for device in self.gpu_manager.preferred_devices():
                ...
                try:
                    trained_ok = self._run_training_loop(train_loader, val_loader, len(feature_cols), num_groups, channels, kernel_size, dropout, self.embedding_dim, epochs, learning_rate, device, timeframe, status_callback)
    ...
    def _run_training_loop(self, train_loader, val_loader, num_features, num_groups, channels, kernel_size, dropout, embedding_dim, epochs, learning_rate, device, timeframe, status_callback):
        model = CNNRegressor(num_features, channels, kernel_size, dropout, num_groups, embedding_dim, len(self.quantiles)).to(device)
        optimizer = torch.optim.Adam(model.parameters(), lr=learning_rate)
        loss_fn = QuantilePinballLoss(self.quantiles).to(device)
    ...
    def save_model(self, model_dir, filename_stem, feature_cols, horizon, timeframe, num_groups, group_id_categories, sequence_length):
        ...
        try:
            ...
            metadata = {
                "model_type": "torch.CNNRegressor", "timeframe": timeframe, "model_path": str(model_path),
                ...
                "architecture": {"num_features": len(feature_cols), "channels": self.channels, "kernel_size": self.kernel_size, "dropout": self.dropout, "embedding_dim": self.embedding_dim, "num_quantiles": len(self.quantiles)},
                "quantiles": self.quantiles, "median_quantile_index": Config.MEDIAN_QUANTILE_INDEX,
            ...
            self._log(f"✅ CNN model saved: {model_path}")
            return True
        except Exception as exc:
...
class CNNTrainerGUI(CTk):
    ...
    def _create_parameters_section(self, parent):
        ...
        self.channels = tk.StringVar(value=",".join(str(c) for c in Config.DEFAULT_CHANNELS))
        self.kernel_size = tk.StringVar(value=str(Config.DEFAULT_KERNEL_SIZE))
        ...
        CTkLabel(row2, text="Channels:", width=70).grid(row=0, column=0, padx=(0, 3), sticky="w")
        CTkEntry(row2, textvariable=self.channels, width=90).grid(row=0, column=1, padx=(0, 10), sticky="w")
        CTkLabel(row2, text="Kernel:", width=70).grid(row=0, column=2, padx=(0, 3), sticky="w")
        CTkEntry(row2, textvariable=self.kernel_size, width=60).grid(row=0, column=3, padx=(0, 10), sticky="w")
        ...
        ctk.CTkCheckBox(param_frame, text="Auto-tune seq len / channels / dropout per timeframe (recommended)", variable=self.auto_tune_var, font=("Ubuntu", 10)).pack(anchor="w", pady=(4, 0))
    ...
    def start_training(self):
        ...
        def training_task():
            try:
                ...
                self.gui_logger.log("🚀 Starting Per-Timeframe CNN Training")

                # Pass GUI flags to Data Preparer
                ...
                gui_sequence_length = int(self.sequence_length.get())
                gui_channels = [int(c.strip()) for c in self.channels.get().split(',') if c.strip()]
                gui_kernel_size = int(self.kernel_size.get())
                gui_batch_size = int(self.batch_size.get())
                gui_dropout = float(self.dropout.get())
                ...
                for timeframe in selected_timeframes:
                    ...
                    self.update_status(f"[{timeframe}] Training CNN model...")
                    trainer = StreamingCNNTrainer(self.gui_logger.log, timeframe=timeframe)
                    ...
                    success = trainer.train_single_timeframe(
                        ...
                        sequence_length=gui_sequence_length, channels=gui_channels, kernel_size=gui_kernel_size,
                        dropout=gui_dropout, batch_size=gui_batch_size, status_callback=self.update_status, auto_tune_architecture=auto_tune
                    )
                    if success and not trainer.stop_flag:
                        filename_stem = f"{Config.MODEL_PREFIX}_{timeframe}"
                        trainer.save_model(model_folder, filename_stem, self.data_preparer.feature_cols, horizon, timeframe, len(self.data_preparer.group_id_categories), self.data_preparer.group_id_categories, trainer.sequence_length)
